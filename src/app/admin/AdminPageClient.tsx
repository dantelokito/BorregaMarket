...
import { getCatalog, getAdminProviders, getAuditLog } from "@/lib/api/admin";
...
import { ProviderTableF10 } from "@/components/admin/ProviderTableF10";
...
export function AdminPageClient() {
  ...
  useEffect(() => {
    if (tab !== "catalogs" || !activeCatalog || activeCatalog === "products") return;
    setLoading(true);
    setError("");
    getCatalog(activeCatalog)
      ...
      .catch((err) => {
        setCatalogData([]);
        if (err instanceof ApiError) setError(mapF10ApiError(err, "module"));
      })
      ...
      {tab === "providers" && (
        loading ? (
          ...
          <ProviderTableF10
            providers={providers}
            onChange={(next) =>
              setProviders((prev) => prev.map((p) => (p.id === next.id ? { ...p, ...next } : p)))
            }
            onError={setError}
          />
        )
